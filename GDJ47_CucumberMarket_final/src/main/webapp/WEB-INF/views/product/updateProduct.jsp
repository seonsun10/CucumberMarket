            ...
            <div class="col-lg-9">
              <div class="box">
                ...
                <form action="${path }/product/updateProduct.do" method="post" enctype="multipart/form-data">	
						...
						placeholder="상품 제목" value="${title }" required><br>
						<label for="exampleInputEmail1">상품 설명</label>
						<textarea class="form-control" name="proContent" rows="6" placeholder="상품 상세 설명" required>${content}</textarea><br>
						<div class="row">
		                    <div class="col-md-4">
						...
						placeholder="상품 가격" value="${price }" required"><br>
						
						<label for="exampleInputEmail1">상품 상태</label><br>
						<label class="radio-inline">
						  <c:if test="${proStatus eq 'y' }">
						  <input type="radio" name="proStatus" id="inlineRadio1" value="y" checked> 중고상품
						...
						<input type="hidden" id="no" name="no" value="${proNo}"/>
						
						
                    <button type="submit" onclick="fn_aaa();" class="btn btn-primary"></i>상품 수정 하기</button>
                  </div>
                </form>
